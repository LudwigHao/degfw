...
_ss_sh_link="https://raw.githubusercontent.com/LudwigHao/degfw/master/ssr.sh"
...
if [[ -f /usr/bin/apt-get || -f /usr/bin/yum ]] && [[ -f /bin/systemctl ]]; then
	...
	echo -e "\n  ${red}当前脚本${none} 不支持你的系统。 ${yellow}(-_-) ${none}\n" && exit 1
...
shadowsocks_port_config() {
	...
	while :; do
		...
		read -p "$(echo -e "请指定一个$yellow Shadowsocks $none端口 [${magenta}1-65535$none]...(默认端口: ${cyan}${random}$none):") " ssport
...
uninstall() {
	if [[ -f $backup ]]; then
		...
		if [[ $is_uninstall ]]; then
			...
			echo
			echo -e "$green 卸载成功$none"
			echo
		fi

	else
		echo
		echo -e "$red 未安装脚本，无法卸载$none"
		echo
	fi
}
